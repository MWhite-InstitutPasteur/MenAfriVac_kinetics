...
int main(int argc, char** argv)
{
	...
	theta.prior_mu[2]    = 7.542732;
	theta.prior_tau[2]   = 2.476895;

	theta.prior_k[2]     = 4.091918;
	theta.prior_theta[2] = 0.1478627;
	...
	for (int n = 0; n<N_part; n++)
	{
		...
		for (int j = 0; j<N_t; j++)
		{
			if (AB_data_read[n][18 + j] > -0.5)
			{
				part[n].tt.push_back(AB_data_read[n][10 + j]);
				part[n].AB.push_back(AB_data_read[n][18 + j]);

				part[n].lAB.push_back(log(AB_data_read[n][18 + j]));
	...
	for (int n = 0; n<N_part; n++)
	{
		...
		part_MCMC[n].par_vec[4] = part[n].logitrho_prim;
	...
	for (int n = 0; n<N_part; n++)
	{
		cout << n << "\t" << "beta_prim: " << part[n].lbeta_prim << "\t" << "t_short: " << part[n].lt_short << "\t" << "t_long: " << part[n].lt_long << "\t" 
			              << "t_IgG: " << part[n].lt_IgG << "\t" << "rho_prim: " << part[n].logitrho_prim << "\t" << "Ab_0: " << part[n].lAb_0 << "\t" << "logL: " << part[n].lhood << endl;
	}
	...
	for (int n = 0; n<N_part; n++)
	{
		local_MCMC_Stream << part[n].beta_prim << "\t" << part[n].t_short << "\t" << part[n].t_long << "\t" << part[n].t_IgG << "\t" << part[n].rho_prim << "\t" << part[n].Ab_0 << "\t" << part[n].lhood << "\t";
	}
	...
	for (int mc = 1; mc<N_mcmc; mc++)
	{
		...
		for (int n = 0; n < N_part; n++)
		{
			...
			part_p1[n].logitrho_prim  = part_MCMC[n].par_vec_test[4];
...
//	cout << "Ab_0   " << p->Ab_0 << endl;
//	cout << "beta_prim   " << p->beta_prim << endl;
//	cout << "t_short   " << log(2)/p->r_short << endl;
//	cout << "t_long   " << log(2) / p->r_long << endl;
//	cout << "t_IgG   " << log(2) / p->r_IgG << endl;
//	cout << "rho_prim   " << p->rho_prim << endl;
//	cout << "t_prim_vac   " << p->t_prim_vac << endl;
	...
	for (int j = 0; j < p->N_sam; j++)
	{
		...
		//cout << "model AB = " << model_AB[j] << "    tt = " << p->tt[j] << endl;
...
double mix_like_n(part_n* p, params* theta)
{
	...
	mixlike = -6.43257 + 0.5*log(theta->tau_par[0]*theta->tau_par[1] *theta->tau_par[2] *theta->tau_par[3] * theta->tau_par[4])
		               ...
		               - 0.5*theta->tau_par[4] *(p->logitrho_prim - theta->mu_par[4])*(p->logitrho_prim - theta->mu_par[4]);
