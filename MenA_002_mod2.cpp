...
#define N_loc_par 10          // Number of individual-level parameters
...
struct part_n
{
	...
	double beta_boost_PsA;  // initial drug concentration
	double beta_Hib;        // initial drug concentration
	...
	double rho_boost_PsA;   // half-life of short-lived component of drugs
	double rho_Hib;         // half-life of short-lived component of drugs
	double Ab_0;

	double lbeta_prim_PsA;      // log(initial drug concentration)
	double lbeta_boost_PsA;     // log(initial drug concentration)
	double lbeta_Hib;           // log(initial drug concentration)
...
int main(int argc, char** argv)
{
	...
	////////////////////////////
	// beta_boost_PsA; initial drug concentration
	...
	////////////////////////////
	// beta_Hib; initial drug concentration
	...
	theta.prior_MM[3]     = 10.0;      // Mean value of the parameter in the population
	theta.prior_MM_CV[3]  = 0.25;       // Coefficient of variation in estimate of population - level parameter

	theta.prior_SIG[3]    = 10.0;       // Between Person standard deviation
	...
	theta.prior_MM[4]     = 1000.0;   // Mean value of the parameter in the population
	theta.prior_MM_CV[4]  = 0.25;      // Coefficient of variation in estimate of population - level parameter

	theta.prior_SIG[4]    = 1000.0;    // Between Person standard deviation
	...
	theta.prior_MM[5]     = 21.0;     // Mean value of the parameter in the population
	theta.prior_MM_CV[5]  = 0.1;     // Coefficient of variation in estimate of population - level parameter

	theta.prior_SIG[5]    = 5.0;      // Between Person standard deviation
	...
	theta.prior_MM[6]     = 0.9;      // Mean value of the parameter in the population
	theta.prior_MM_CV[6]  = 0.1;      // Coefficient of variation in estimate of population - level parameter

	theta.prior_SIG[6]    = 0.1;      // Between Person standard deviation
	theta.prior_SIG_CV[6] = 0.25;     // Coefficient of variation in Between Person sd


	////////////////////////////
	// rho_boost_PsA;  proportion of short-lived component at start

	theta.prior_MM[7]    = 0.9;      // Mean value of the parameter in the population
	theta.prior_MM_CV[7] = 0.1;      // Coefficient of variation in estimate of population - level parameter

	theta.prior_SIG[7]    = 0.1;      // Between Person standard deviation
	...
	for (int n = 0; n<N_part; n++)
	{
		...
		for (int j = 0; j<N_t; j++)
		{
			if (AB_data_read[n][17 + j] > -0.5)
			{
				...
				part[n].AB.push_back(AB_data_read[n][17 + j]);
		...
		part[n].beta_boost_PsA = genunf(100.0, 200.0);
		part[n].beta_Hib       = genunf(100.0, 200.0);
		...
		part[n].rho_boost_PsA  = genunf(0.8, 0.9);
		part[n].rho_Hib        = genunf(0.8, 0.9);
		part[n].Ab_0           = genunf(0.0, 0.01);

		part[n].lbeta_prim_PsA  = log(part[n].beta_prim_PsA);
		part[n].lbeta_boost_PsA = log(part[n].beta_boost_PsA);
		part[n].lbeta_Hib       = log(part[n].beta_Hib);
		...
		part[n].logitrho_boost_PsA = log(part[n].rho_boost_PsA / (1.0 - part[n].rho_boost_PsA));
		part[n].logitrho_Hib       = log(part[n].rho_Hib / (1.0 - part[n].rho_Hib));
	...
	for (int n = 0; n<N_part; n++)
	{
		...
		part_MCMC[n].par_vec[1] = part[n].lbeta_boost_PsA;
		part_MCMC[n].par_vec[2] = part[n].lbeta_Hib;
		part_MCMC[n].par_vec[3] = part[n].lt_short;
		part_MCMC[n].par_vec[4] = part[n].lt_long;
		part_MCMC[n].par_vec[5] = part[n].lt_IgG;
		part_MCMC[n].par_vec[6] = part[n].logitrho_prim_PsA;
		part_MCMC[n].par_vec[7] = part[n].logitrho_boost_PsA;
		part_MCMC[n].par_vec[8] = part[n].logitrho_Hib;
		part_MCMC[n].par_vec[9] = part[n].lAb_0;
	...
	for (int n = 0; n<N_part; n++)
	{
		cout << n << "\t" << "beta_prim_PsA: " << part[n].beta_prim_PsA << "\t" << "beta_boost_PsA: " << part[n].beta_boost_PsA << "\t" << "beta_Hib: " << part[n].beta_Hib 
			      << "\t" << "t_short: " << part[n].t_short << "\t" << "t_long: " << part[n].t_long << "\t" << "t_IgG: " << part[n].t_IgG 
			      << "\t" << "rho_prim_PsA: " << part[n].rho_prim_PsA << "\t" << "rho_boost_PsA: " << part[n].rho_boost_PsA << "\t" << "rho_Hib: " << part[n].rho_Hib
	...
	for (int n = 0; n<N_part; n++)
	{
		local_MCMC_Stream << part[n].beta_prim_PsA << "\t" << part[n].beta_boost_PsA << "\t" << part[n].beta_Hib	<< "\t" 
			              << part[n].t_short << "\t" << part[n].t_long << "\t" << part[n].t_IgG << "\t"
			              << part[n].rho_prim_PsA << "\t" << part[n].rho_boost_PsA << "\t" << part[n].rho_Hib	<< "\t" 
	...
	for (int mc = 1; mc<N_mcmc; mc++)
	{
		...
		for (int n = 0; n < N_part; n++)
		{
			...
			part_p1[n].lbeta_boost_PsA    = part_MCMC[n].par_vec_test[1];
			part_p1[n].lbeta_Hib          = part_MCMC[n].par_vec_test[2];
			...
			part_p1[n].logitrho_prim_PsA  = part_MCMC[n].par_vec_test[6];
			part_p1[n].logitrho_boost_PsA = part_MCMC[n].par_vec_test[7];
			part_p1[n].logitrho_Hib       = part_MCMC[n].par_vec_test[8];
			part_p1[n].lAb_0              = part_MCMC[n].par_vec_test[9];
		...
		for (int n = 0; n < N_part; n++)
		{
			...
			if (local_prior(&part_p1[n]) > -0.5*LARGE)
			{
				...
				part_p1[n].beta_boost_PsA = exp(part_p1[n].lbeta_boost_PsA);
				part_p1[n].beta_Hib       = exp(part_p1[n].lbeta_Hib);
				...
				part_p1[n].rho_boost_PsA = exp(part_p1[n].logitrho_boost_PsA) / (1.0 + exp(part_p1[n].logitrho_boost_PsA));
				part_p1[n].rho_Hib       = exp(part_p1[n].logitrho_Hib) / (1.0 + exp(part_p1[n].logitrho_Hib));
...
double data_like_n(part_n* p, params* theta)
{
	...
	if ((p->prim_vac == 1) && (p->boost_vac == 1))
	{
		for (int j = 0; j < p->N_sam; j++)
		{
			...
			{
				model_AB[j] = model_AB[j] + p->beta_boost_PsA*(           (p->rho_boost_PsA / (p->r_short - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_boost_vac)) - exp(-p->r_short * (p->tt[j] - p->t_boost_vac)))
					                                            + ((1.0 - p->rho_boost_PsA) / (p->r_long - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_boost_vac)) - exp(-p->r_long * (p->tt[j] - p->t_boost_vac))));
			}
		}
	}
	...
	if ((p->prim_vac == 1) && (p->boost_vac == 2))
	{
		for (int j = 0; j < p->N_sam; j++)
		{
			...
			if (p->tt[j] > p->t_boost_vac)
			{
				model_AB[j] = model_AB[j] + p->beta_Hib*(             (p->rho_Hib / (p->r_short - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_boost_vac)) - exp(-p->r_short * (p->tt[j] - p->t_boost_vac)))
					                                        + ((1.0 - p->rho_Hib) / (p->r_long - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_boost_vac)) - exp(-p->r_long * (p->tt[j] - p->t_boost_vac)))    );
			}
		}
	}
	...
	if ((p->prim_vac == 2) && (p->boost_vac == 1))
	{
		for (int j = 0; j < p->N_sam; j++)
		{
			model_AB[j] = model_AB[j] + p->beta_Hib*(            (p->rho_Hib / (p->r_short - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_prim_vac)) - exp(-p->r_short * (p->tt[j] - p->t_prim_vac)))
			  	                                        + ((1.0 - p->rho_Hib) / (p->r_long - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_prim_vac)) - exp(-p->r_long * (p->tt[j] - p->t_prim_vac)))     );

			if (p->tt[j] > p->t_boost_vac)
			{
				model_AB[j] = model_AB[j] + p->beta_prim_PsA*(           (p->rho_prim_PsA / (p->r_short - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_boost_vac)) - exp(-p->r_short * (p->tt[j] - p->t_boost_vac)))
			                                           		   + ((1.0 - p->rho_prim_PsA) / (p->r_long - p->r_IgG))*(exp(-p->r_IgG * (p->tt[j] - p->t_boost_vac)) - exp(-p->r_long * (p->tt[j] - p->t_boost_vac)))   );
...
double mix_like_n(part_n* p, params* theta)
{
	...
	mixlike = -10.10832 + 0.5*log( theta->tau_par[0] * theta->tau_par[1] * theta->tau_par[2] * theta->tau_par[3] * theta->tau_par[4] * theta->tau_par[5] * theta->tau_par[6] * theta->tau_par[6] * theta->tau_par[7] )
		                - 0.5*theta->tau_par[0] *(p->lbeta_prim_PsA     - theta->mu_par[0] )*(p->lbeta_prim_PsA     - theta->mu_par[0])
		                - 0.5*theta->tau_par[1] *(p->lbeta_boost_PsA    - theta->mu_par[1] )*(p->lbeta_boost_PsA    - theta->mu_par[1])
		                - 0.5*theta->tau_par[2] *(p->lbeta_Hib          - theta->mu_par[2])*(p->lbeta_Hib           - theta->mu_par[2])
				        - 0.5*theta->tau_par[3] *(p->lt_short           - theta->mu_par[3] )*(p->lt_short           - theta->mu_par[3])
		                - 0.5*theta->tau_par[4] *(p->lt_long            - theta->mu_par[4] )*(p->lt_long            - theta->mu_par[4])
		                - 0.5*theta->tau_par[5] *(p->lt_IgG             - theta->mu_par[5] )*(p->lt_IgG             - theta->mu_par[5])
		                - 0.5*theta->tau_par[6] *(p->logitrho_prim_PsA  - theta->mu_par[6] )*(p->logitrho_prim_PsA  - theta->mu_par[6])
	                    - 0.5*theta->tau_par[6] *(p->logitrho_boost_PsA - theta->mu_par[6])*(p->logitrho_boost_PsA  - theta->mu_par[6])
						- 0.5*theta->tau_par[7] *(p->logitrho_Hib       - theta->mu_par[7])*(p->logitrho_Hib        - theta->mu_par[7]);
...
double local_prior(part_n* p)
{
	...
	if (p->beta_prim_PsA < 0.0) { return -LARGE; }
	if (p->beta_Hib < 0.0) { return -LARGE; }
...
//	if (p->lt_IgG > 5.298317 ) { return -LARGE; }
	...
	if (p->rho_boost_PsA < 0.0) { return -LARGE; }
	if (p->rho_boost_PsA > 1.0) { return -LARGE; }

	if (p->rho_Hib < 0.0) { return -LARGE; }
	if (p->rho_Hib > 1.0) { return -LARGE; }
